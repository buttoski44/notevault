...
export const Note = ({ note }) => {
    ...
    return (
        <motion.div className={`w-full  sm:w-48 md:w-52 h-40 sm:h-48 md:h-52 shadow-xl p-4 rounded-xl cursor-pointer flex justify-between ${COLORS[color]}`}
        ...
        >
            <div className="flex flex-col justify-between w-full h-full gap-2" onClick={handleClick}>
                <h2 className="text-md sm:text-lg font-bold text-black ">{title ? title : <div className="opacity-[0]">""</div>}</h2>
                <p className="text-xs sm:text-sm w-full h-full overflow-hidden font-semibold ">{tagline}
                </p><p className="text-xs/3 sm:text-xs font-bold">{"" + timeArr[2] + " " + timeArr[1] + ", " + timeArr[3]}</p>
            </div>
